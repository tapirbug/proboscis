...
use super::{FunctionDefinition, GlobalDefinition};
...
impl<'t, 's> NameCheck<'t, 's> {
    pub fn check(
        ...
        functions: &'t [FunctionDefinition<'t, 's>],
        globals: &'t [GlobalDefinition<'t, 's>],
        root_code: &'t [&AstNode<'s>]) -> Result<(), NameError<'t, 's>> {
        let mut check = Self::new(source, functions, globals);
        for root_code in root_code {
    ...
    fn push_fn_scope(&mut self, fun: &'t FunctionDefinition<'t, 's>) {
        ...
        for arg in fun.args().elements() {
            ...
            let arg = arg.atom().expect("only plain identifiers supported as arguments");
    ...
    fn pop_scope(&mut self) {
        self.scope_vars.pop().expect("Tried to pop scope but scopes are empty");
    }

    fn check_names(&self, code: &'t AstNode<'s>) -> Result<(), NameError<'s, 't>> {
        match code {
            ...
            AstNode::Atom(atom) => match atom.token().kind() {
                ...
                TokenKind::FloatLit | TokenKind::IntLit | TokenKind::StringLit | TokenKind::Comment | TokenKind::Ws | TokenKind::LeftParen | TokenKind::RightParen => Ok(()),
    ...
    fn check_fn_ident(&self, ident_atom: &'t Atom<'s>) -> Result<(), NameError<'s, 't>> {
        let ident_str = ident_atom.source_range().of(self.source).source();
        for defined_fn in self.functions {
            let defined_fn_ident = defined_fn.name().source_range().of(defined_fn.source()).source();
        ...
        Err(NameError::UndefinedFunctionName { source: self.source, name: ident_atom })
    }

    fn check_variable_ident(&self, ident_atom: &'t Atom<'s>) -> Result<(), NameError<'s, 't>> {
        let ident_str = ident_atom.source_range().of(self.source).source();
        let is_defined_scope_var = self.scope_vars.iter().flat_map(|v| v.iter()).find(|&&scope_var_ident| scope_var_ident == ident_str).is_some();
        if is_defined_scope_var {
            return Ok(())
        }
        for defined_global in self.globals {
            let defined_global_ident = defined_global.name().source_range().of(defined_global.source()).source();
        ...
        Err(NameError::UndefinedVariableName { source: self.source, name: ident_atom })
    }

    fn check_invocation(&self, invocation: &'t List<'s>) -> Result<(), NameError<'s, 't>> {
        ...
        let function_ident = function_ident.atom()
            .filter(|a| matches!(a.token().kind(), TokenKind::Ident))
            .ok_or_else(|| NameError::MalformedFunctionName { source: self.source, name: function_ident })?;
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            NameError::UndefinedFunctionName { source, name } => {
                writeln!(f, "reference to undefined function `{}`:", name.source_range().of(source).source())?;
            ...
            NameError::UndefinedVariableName { source, name } => {
                writeln!(f, "reference to undefined place `{}`:", name.source_range().of(source).source())?;
                writeln!(f, "{}", name.fragment(source).source_context())
            }
        }
    }
